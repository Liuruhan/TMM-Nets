...
class BasicDataset(Dataset):
    def __init__(self, imgs_dir, n_classes, scale=1, patch_scale=8, resample=True):
    ...
    @classmethod
    def preprocess(cls, pil_img, scale, patch_scale):
        w, h = pil_img.size
        ...
        #print(img_return.shape[0], patch_scale, int(img_return.shape[1]/patch_scale), patch_scale, int(img_return.shape[2]/patch_scale))
        img_return = np.reshape(img_return, (img_return.shape[0], patch_scale, int(img_return.shape[1]/patch_scale), patch_scale, int(img_return.shape[2]/patch_scale)))
        img_return = np.swapaxes(img_return, 2, 3)
        return img_return
    ...
    def __getitem__(self, i):
        ...
        img = self.preprocess(img, self.scale, self.patch_scale)
