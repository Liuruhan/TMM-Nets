...
class BasicDataset(Dataset):
    def __init__(self, imgs_dir, masks_dir, n_classes, scale=1, resample=True):
    ...
    def __getitem__(self, i):
        ...
        return {'image': torch.from_numpy(img).type(torch.FloatTensor), 'mask': torch.from_numpy(mask).type(torch.FloatTensor), 'target': torch.from_numpy(label_one_hot).type(torch.FloatTensor)}# label.type(torch.LongTensor)} #}
