...
from patch_models.resnet import Resnet18, Resnet34, Resnet50, Resnet101, Resnet152, Resnext50_32x4d, Resnext101_32x8d, wide_Resnet50_2, wide_Resnet101_2
#from dataset import BasicDataset
from dataset_patches import BasicDataset
from torch.utils.data import DataLoader
import matplotlib.pyplot as plt
from segLoss import SegmentationLosses
ultra_DR_learning_rate = 0.000002
...
def model_selected(model_name, n_classes):
    if model_name == 'scse_res':
        ...
        if model_name == 'resnet18':
            ...
            model = Resnext50_32x4d(num_classes=n_classes)
        elif model_name == 'resnext101_32x8d':
            resnet = models.resnext101_32x8d(pretrained=True)
            model = Resnext101_32x8d(num_classes=n_classes)
        elif model_name == 'wide_resnet50_2':
            resnet = models.wide_resnet50_2(pretrained=False)
            model = wide_Resnet50_2(num_classes=n_classes)
        elif model_name == 'wide_resnet101_2':
            resnet = models.wide_resnet101_2(pretrained=True)
            model = wide_Resnet101_2(num_classes=n_classes)
...
def train_k_fold(args, model_list, k):
    ...
    train = BasicDataset(tr_dir_img, args.num_classes, scale=1, patch_scale=8, resample=True)
    val = BasicDataset(val_dir_img, args.num_classes, scale=1, patch_scale=8, resample=False)
    n_val = len(val)
    n_train = len(train)
...
if __name__ == "__main__":
    model_list = ['resnet18', 'resnet34', 'resnet50','wide_resnet50_2']#'mobilenet_v2', 'inception_v3', 'densenet121', 'densenet161']
    #'resnet18',
           #'resnet18', 'resnet34',
    ...
    tr_dir_img = "./DR_rename_dataset/train/"
    val_dir_img = "./DR_rename_dataset/test/"

    train = BasicDataset(tr_dir_img, args.num_classes, 1, 8, True)
    val = BasicDataset(val_dir_img, args.num_classes, 1, 8, False)
    n_val = len(val)
    n_train = len(train)
    print('samples:', n_train, n_val)
    ...
    for i in range(len(model_list)):
        ...
        print(model_list[i])
        train_net(model_list[i], args.num_classes, train_loader, val_loader, 23, load_path)
